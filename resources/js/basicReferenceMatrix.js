...
function listReviewReferenceMatrix(){
	...
	$.get("../basicReferenceMatrix/referenceMatrixList.htm",function(response){
		...
		if(response.status=="LIST_SUCCESS"){
			...
			$("#listApiUserTableMatrix_length").show();
...
function listReviewReferenceMatrixResponse(response){
	...
	html+='<td style="word-break:break-all;">'+data[i].nlpQuery+'</td>';
	}else{
	html+='<td></td>';
	}
	html+='<td>'+data[i].status+'</td>';
    html+='<td><span style="margin-left: -2px;"><span><button type="button" class="btn btn-xs AdminInList" title="Edit" onclick="editReviewReferenceMatrixData('+data[i].id+')" style="margin-left: -9px;"><span class="glyphicon glyphicon-pencil"></span></button></span>';
	html+='<span><button type="button" style="right=-1px;" class="btn btn-xs AdminInList" title="Delete"   onclick="deleteReviewReferenceMatrixModal('+data[i].id+')"><span class="glyphicon glyphicon-trash"></span></button></span></span></td>';
	html+='</tr>';
	}
	html+='</tbody>';
	html+='</table>';
...
function addReviewReferenceMatrix(){
	$.ajax({
		...
		success : function(data) {
			...
			html+='<label class="control-label" style="width:400px" for="IndustryType">Industry Type<font style="color: red">*</font>';
			html+='<select id="selectIndustryType" class="form-control input-sm" style="width: 250px;float: right;" onchange="segmentType()">';
			...
			//segment
			html+='<br><label id="segmentType" style="width:400px" for="SegmentType">Segment Type<font style="color: red">*</font> <select class="form-control input-sm"  style="width: 250px;float: right;" id="selectedSegmentType" disabled>';
			...
			html+='<input type="text" style="width: 100%;" class="form-control input-sm" id="nlpQueryName" onkeydown="autoSearchQueryName()" >';
			html+='</div>';
			...
			html+=' <label for="keywordsPrefix" class="col-xs-12 row">Keyword Prefix</label>';
			...
			html+='<input type="text" style="width: 100%;" class="form-control input-sm" id="keywords" onkeydown="autoSearchKeywords()">';
			html+='</div>';
			html+='</div>';
...
function segmentType(){
	...
	$.ajax({
		...
		success: function(data){ 
			...
			var html="";
			html+=	'<label id="segmentType" class="control-label" style="width:400px"; for="SegmentCategory">Segment Type<font style="color: red">*</font> <select class="form-control input-sm" id="selectSegmentType" style="width: 250px;float: right;" onchange="organizationType()">';
...
var delay = (function(){
 ...
 function departmentType()
  {
	...
	$.ajax({
		...
		success: function(data){ 
			...
			html+='<label id="departmentType" style="width:400px"; for="DepartmentCategory">Department Type<font style="color: red">*</font>';
 ...
 function queryKeyword()
 {
	...
	$.ajax({
		...
		success: function(data){ 
			console.log(data);
			var html="";
			html+='<label id="prefixDistances" for="prefixDistance" class="col-xs-12 row">Prefix Distance</label>';
			html+='<select class="form-control input-sm" style="width: 120px;" id="prefixDistance" >';
			html+=	'<option value="0">Select</option>';
			for(var i=0;i<data.length;i++){
 ...
 function deleteReviewReferenceMatrixModal(id){
	 $('#deleteBasicReferenceModalNew').html('');
	 var html="";
	 html+='<div id="deleteBasicReferenceModal" class="modal fade">';
     ...
     html+='</div>';
    html+='</div>';
    $('#deleteBasicReferenceModalNew').append(html);
    $('#deleteBasicReferenceModal').modal('show');
 } 
	 ...
	 //if(confirm("Are you sure want to delete selected item?")){
			$.post("../basicReferenceMatrix/delete.htm?id="+id,function(response){
				if(response.status=="DELETE_SUCCESS"){
					...
					$('#deleteBasicReferenceModal').modal('hide');
					//$('#geoCountryTabButtons').show();
					$('#listApiUserTableMatrix').dataTable().columnFilter({"lengthMenu": [ [10, 25, 50, -1], [10, 25, 50, "All"] ],"bScrollCollapse":true});
					$("#listApiUserTableMatrix_filter").hide();
					$addAndEditReviewReferenceMatrixDiv.hide();
					$('#listApiUserTabMatrix').show();
					$('#loadMaskDiv').unmask();
	...
	if(nearBy=="Select"){
		...
		$.post("../basicReferenceMatrix/save.htm",JSONObject,function(response){
			...
			if(response.status=="SAVE_ERROR"){
				...
				$('#page-wrapper').unmask();
				$('#addReviewReferenceMatrixSuccessDiv').show(600);
 ...
 function appendReferenceMatrixAddOrEditForm(divId,method){
	...
	function referenceMatrixEditFormHtml(response){
		...
		html += '<span style="color: #a94442" id="editDepartmentType-span-Error" class="help-inline"></span>';
		...
		html += '<span style="color: #a94442" id="editKpi-span-Error" class="help-inline"></span>';
		...
		html+='<input type="text" class="form-control input-sm" id="editReference" style="width:250px;" value="'+editReferenceKpiMappingUI.referenceCategory+'"  onkeydown="editAutoSearchReference()">';
		html+='</div>';
		...
		html+='<input type="text" class="form-control input-sm" id="editNlpQueryName" style="width: 100%;"  value="'+editReferenceKpiMappingUI.nlpQueryName+'" onkeydown="editAutoSearchQueryName()">';
		html+='</div>';
		...
		html+='<input type="text" class="form-control input-sm" id="editKeywordsPrefix" onkeydown="editAutoSearchKeywordsPrefix()" value="'+editReferenceKpiMappingUI.nearByKeyword+'" >';
		}else{
		...
		//keywords
		html+='<div class="ui-widget col-xs-3">';
		html+=' <label for="editKeywords" class="col-xs-12 row">Keywords</label>';
	...
	function updateReferenceMatrix(id){
		...
		$.post(referenceMatrixUrl+"update.htm",JSONObject,function(response){
			...
			if(response.status=="UPDATE_SUCCESS"){
				$('#editReferenceMatrixSuccessDiv').show(600);
				$('#page-wrapper').unmask();
				listReviewReferenceMatrix();
