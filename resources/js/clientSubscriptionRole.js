...
function PopUp(){
	alert("test");
	$("#saveClientSubscriptionType").modal('show');
}
...
function addNewRole(){
	alert("a");
	var $form = $('#addClientSubscription');
	$form.trigger("reset");
...
*********************************Next Button*******************************************************************************
***************************************************************************************************************************//*
$('#addClientSubscription').unbind('submit').bind('submit',function() {alert("b");
	...
	$.post("../clientSubscriptionRole/addFeatures.htm",JSONObject,function(response){
		$('#addRoleErrorDiv').hide();
		var $form = $('#addClientSubscription');
		$form.find('.form-group').removeClass('has-error has-feedback');
		$form.find('.help-inline').empty();
...
function addFeatures(response){alert("sgm");
...
function getFeatures(featureUrl){alert("c");
	$('#featuresDiv').html('');
	var JSONObject = {};
	...
	var html = "";
	$.post("../clientSubscriptionRole/getFeatures.htm",JSONObject,function(response){alert("sgm1");
		if(response.status=="LIST_SUCCESS"){alert("sgm2");
			...
			html+='</tr>';
			for(var j=0;j<featureMaster.length;j++){alert("sgm3");
...
function saveRoleFeatures(form){alert("d");
	...
	var roleJsonObject = {"parentRole":$('#roleId option:selected').text(),"role":$.trim($('#role').val()),
							"description":$.trim($('#description').val()),"featureIds":arr};
	$.ajax({
		type:"POST",
		url:"../roleFeature/save.htm",
		...
		success:function(response){
			if(response.status=="SAVE_ERROR"){
				...
				$('#addClientSubscription').trigger("reset");
			}else{
				$('#loadMaskDiv').mask(response.status+"***************"+response.statusText);
			}
...
**********************E d i t****************************************************************************************************************
**********************************************************************************************************************************************//*    
  function editRoleFeatue(){alert("e");
  ...
  **********************************************************************************************************************************************//*  
  ...
  function editMapRoleFeatures(response){
	...
	function getFeaturesToUpdate(featureUrl,parentRoleId,featureIdsToUpdate,roleId){alert("h");
		...
		$.post("../roleFeature/getFeatures.htm",JSONObject,function(response){
			if(response.status=="LIST_SUCCESS"){
				...
				html+=	'<tr>';
				html+=		'<th><input type="checkbox" id="map_SelectAll_'+convertedFeatureUrl+'_ChkBox" style="margin-left:24px;" onclick="mapSelectAll(this)">&nbsp;'+featureUrl+'</th>';
				html+='</tr>';
  ...
  function updateRoleFeatures(form){alert("i");
	  ...
	  var count = 0;
		...
		var roleJsonObject = {"roleId":$('#roleIdToUpdate').val(),"featureIds":arr,"parentRoleId":$('#parentRoleId').val()};
				$.ajax({
				type:"POST",
				url:"../roleFeature/mapFeatures.htm",
				contentType:"application/json",
				data:JSON.stringify(roleJsonObject),
				success:function(response){
  ...
  function mapSelectAll(checkBox){alert("i");
		 var checkBoxId = checkBox.id;
		 $(document).on('click',"#"+checkBoxId,function(){
			 checkBoxId = checkBoxId.replace('map_SelectAll_','');
		       $('.map_'+checkBoxId).prop('checked', $(this).is(':checked'));
		     });
	}
	function mapSelectCheckBox(checkBoxId){alert("j");
		var id = checkBoxId.id;
		$(document).on('click',".map_"+id,function(){
		       if($('.map_'+id+':checked').length == $('.map_'+id).length) {
		         $('#map_SelectAll_'+id).prop('checked', true);
		       }
		       else {
		    	   $('#map_SelectAll_'+id).prop('checked', false);
		       }
		 });
	}
  ...
  **********************V i e w ****************************************************************************************************************
  **********************************************************************************************************************************************//*
  function showRoleFeature(){alert("k");
		roleFeatureSelectAllChkBox = [];
		RoleFeaturecheckBoxLength();
	  	if(roleFeatureSelectAllChkBox.length>1){
  ...
  function showRoleFeatureResponse(response){alert("l");
...
**************************L i s t************************************************************************************************************
**********************************************************************************************************************************************//*
function listCustomRoles(){alert("m");
...
function listCustomRolesResponse(response){alert("n");
	$('#listAllRoleFeatures').html('');
		var html = "";
		html+="<hr>";
...
function RoleFeaturecheckBoxLength(){alert("p");
 ...
 ****************************D e l e t e******************************************************************************************************
 **********************************************************************************************************************************************//*   
 function deleteRoleFeatues(){alert("q");
	 RoleFeaturecheckBoxLength();
