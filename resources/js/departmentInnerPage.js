...
function filterDepartmentInnerPage(departmentName){
	...
	$.ajax({
		type:"POST",
		url:"../departmentInnerPage/departmentName.htm",
		...
		success:function(response){
			...
			window.location.href = "../departmentInnerPage/page.htm?organizationId="+organizationId+"&fromDate="+fromDate+"&toDate="+toDate+"&isBreadCrumb="+isBreadCrumb+"&departmentId="+departmentId+"&isDashBoard="+isDashBoard;
...
function departmentInnerPageChart(){
	...
	$.ajax({
		type:"POST",
		url:"../departmentInnerPage/innerChart.htm",
		contentType:"application/json",
...
function listDepartmentInnerPageReviews(organizationId){
	...
	$.ajax({
		type:"POST",
		url:"../departmentInnerPage/showReviewsForDepartments.htm",
		contentType:"application/json",
		...
		success:function(response){
			...
			if(response.length>0){
				...
				("#page-selection").html(" ");
				("#departmentInnerPageSummaryReviewsSearch").html(" ");
			}
		},
...
function listDepartmentInnerPageReviewsResponse(response,searchTextVal){
	...
	for(var i=0;i<list.length;i++){
		...
		if(list[i].highlightedReviewContent!=null && list[i].highlightedReviewContent!="" && list[i].reviewLanguage!="Unknown" && list[i].reviewLanguage!="N/A" && list[i].reviewLanguage!=null && list[i].reviewLanguage!="" && list[i].reviewLanguage.toUpperCase()!="ENGLISH" && list[i].reviewLanguage.toUpperCase()!="EN"){
			//original review
		...
		if(list[i].sourceName.toLowerCase()=="tripadvisor" && list[i].fromApi==true){
			...
			tempHtml += '<span>Source Rating </span><span data-review-rating="'
							...
							/*if(list[i].respondStatus==true){
								html += '<span style="float:right;color:red;"><a onclick="getResponds('+list[i].id+')" href="#">Replied To Review</a></span>';
							}else{
								html += '<span id="repliedToReview_'+list[i].id+'" style="display:none;float:right;color:red;"><a onclick="getResponds('+list[i].id+')" href="#">Replied To Review</a></span>';
							}*/
							tempHtml += '</div>';
		}else{
