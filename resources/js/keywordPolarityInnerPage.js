...
var innerPageKeywordChartSummaryResponse;
var keywordInnerPageBarChartId = "KeywordFactorColumnSummaryStacked";
var keywordInnerPageStartPage= 0;
var keywordInnerPageEndPage = 10;
var keywordInnerPage = 1;
var keywordInnerPageUrl = "../keywordPolarityInnerPage/";
var keywordInnerPageStartPage= 0;
var keywordInnerPageEndPage = 10;
var keywordInnerPage = 1;
...
$(document).ready(function(){
	...
	listKeywordInnerPageReviews(organizationId);
});

var keywordInnerPageFromDate = "keywordInnerPageFromDate";
var kewordInnerPageToDate = "altToDate";
var keywordApplyButton = "keywordInnerPageApply";
$("#keywordInnerPageFromDate").datepicker({
	defaultDate : "+1w",
	changeMonth : true,
	numberOfMonths : 1,
	dateFormat:'d M yy',
	...
	onClose : function(selectedDate) {
		$("#altToDate").datepicker("option", "minDate", selectedDate);
		dateValidation(keywordInnerPageFromDate,kewordInnerPageToDate,keywordApplyButton);
	}
});

$("#altToDate").datepicker({
	defaultDate : "+1w",
	changeMonth : true,
	numberOfMonths : 1,
	dateFormat:'d M yy',
	altField: "#altKeywordInnerPageToDate",
	altFormat: "yy-mm-dd",
	maxDate: new Date,
	onClose : function(selectedDate) {
	$("#keywordInnerPageFromDate").datepicker("option", "maxDate", selectedDate);
	dateValidation(keywordInnerPageFromDate,kewordInnerPageToDate,keywordApplyButton);
	}

});
function filterKeywordInnerPage(){
	loadingForDashBoard();
	var fromDate = $('#altFromDate').val();
	var toDate = $('#altKeywordInnerPageToDate').val();
	var organizationId = $('#organizationName option:selected').val();
	var kpiId = $('#innerPageKeywordSummaryKpiId').val();
	var departmentId  = $('#innerPageKeywordSummaryDepartmentId').val();
	var keywordId = $('#innerPageKeywordSummaryKeywordId').val();
	var isBreadCrumb = $('#innerPageKpiBreadCrumb').val();
	window.location.href = "../keywordPolarityInnerPage/page.htm?kpiId="+kpiId+"&departmentId="+departmentId+"&keywordId="+keywordId+"&isBreadCrumb="+isBreadCrumb+"&organizationId="+organizationId+"&fromDate="+fromDate+"&toDate="+toDate;

}


function keywordInnerPageChart(){
	loadingForDashBoard();
	var departmentId = $('#innerPageKeywordSummaryDepartmentId').val();
	var kpiId = $('#innerPageKeywordSummaryKpiId').val();
	var keywordId = $('#innerPageKeywordSummaryKeywordId').val();
	$.ajax({
		type:"POST",
		url:keywordPolarityInnerPage+"innerChart.htm",
		contentType:"application/json",
		data:JSON.stringify({'departmentId':departmentId,'kpiId':kpiId,'keywordId':keywordId}),
		success:function(response){
			var list = response.successObject.list;
			innerPageKeywordChartSummaryResponse = list;
			keywordInnerPageChartResponse(list,keywordInnerPageStartPage,keywordInnerPageEndPage);
...
function nextInnerPageKeywordSummaryChart(){
	keywordInnerPageStartPage=keywordInnerPageStartPage+10;
	keywordInnerPageEndPage=keywordInnerPageEndPage+10;
	keywordInnerPageChartResponse(innerPageKeywordChartSummaryResponse,keywordInnerPageStartPage,keywordInnerPageEndPage);
}
function previousInnerPageKeywordSummaryChart(){
	keywordInnerPageStartPage=keywordInnerPageStartPage-10;
	keywordInnerPageEndPage=keywordInnerPageEndPage-10;
	keywordInnerPageChartResponse(innerPageKeywordChartSummaryResponse,keywordInnerPageStartPage,keywordInnerPageEndPage);
}
...
function keywordInnerPageChartResponse(list,kpiInnerPageStartPage,kpiInnerPageEndPage){
	...
	if(listLength<=10){
		$('#innerPageKeywordSummaryNextButton').prop('disabled',true);
		//$('#innerPageKeywordSummaryNextButton').css('background-color','red');
	}
	if(listLength>0){
	...
	if(kpiInnerPageEndPage>=list.length){
		...
		$('#innerPageKeywordSummaryNextButton').prop('disabled',false);
	}
	
	if(kpiInnerPageStartPage<=0){
		$('#innerPageKeywordSummaryPreviousButton').prop('disabled',true);
	}else{
		$('#innerPageKeywordSummaryPreviousButton').prop('disabled',false);

	}
	populateBarChart(keywordInnerPageBarChartId,datesArray,positiveArray,negativeArray,neutralArray,"Keyword Factor");
...
/*$('#keywordInnerPageSummaryReviewsSearch').keyup(function(e){
		 keywordSearchText = $(this).val();
		 $('#keywordInnerPageSummaryReviews').html('');
		 listKeywordInnerPageReviews(keywordInnerPage);
});*/


var stoppedTyping;
function keywordInnerPageSearch(){
	...
	stoppedTyping = setTimeout(function(){
		...
		var searchKey = $("#keywordInnerPageSummaryReviewsSearch").val();
		...
		$('#page-selection')
				.bootpag({
			        ...
			        maxVisible: 5
							...
							$('#keywordInnerPageSummaryReviews').append(tempHtml);
							$('span.stars').stars();	
...
function listsearchedReviewsResponse(resultListReviews, searchText) {
	...
	if (resultListReviews.length > 0) {
		...
		$('#keywordInnerPageSummaryReviews').empty();
	}else {
...
function listKeywordInnerPageReviews(organizationId){
	loadingForDashBoard();
	getSentimentPolarityList();
	...
	$.ajax({
		type:"POST",
		url:"../keywordPolarityInnerPage/showReviewsForKeywords.htm",
		contentType:"application/json",
		data:JSON.stringify({fromDate:fromDate, toDate:toDate, organizationId:organizationId, departmentId:departmentId, kpiId:kpiId, keywordId:keywordId}),
		success:function(response){
			console.log(response);
			var result = response.successObject.reviewDetailsKeyword;
			var polarity = response.successObject.polarity;
			console.log("result is:"+JSON.stringify(result));
			var response = result;
			$('#keywordTotalReviewCount').text(response.length);
			...
			if(response.length>0){
				...
				$("#KeywordreviewHeaderId").html("");
				$("#KeywordreviewHeaderId").append("<h4 style='color:red'>No "+polarity+" Review Found !!</h4>").show();
				$("#page-selection").html(" ");
				$("#searchIconId").hide();
			}
		},
...
/*function listKeywordInnerPageReviews(keywordInnerPage){
	loadingForDashBoard();
	var innerPageKpiId = $('#innerPageKeywordSummaryKpiId').val();
	var departmentId = $('#innerPageKeywordSummaryDepartmentId').val();
	var keywordId = $('#innerPageKeywordSummaryKeywordId').val();
	var keywordSearchTextVal = $.trim($('#keywordInnerPageSummaryReviewsSearch').val());
	var JSONObject = {'searchText':keywordSearchTextVal,'kpiId':innerPageKpiId,'departmentId':departmentId,'keywordId':keywordId};
	$.post(keywordInnerPageUrl+"reviewSearch.htm?page="+keywordInnerPage,JSONObject,function(response){
		if(response.status=="SUCCESS"){
			var reviews = response.successObject.reviewsForInnerPage;
			if(reviews.length>0){
				$('#keywordInnerPageSummaryReviews').html('');
				var html = listKeywordInnerPageReviewsResponse(response,keywordSearchTextVal);
				$('#keywordInnerPageSummaryReviews').append(html);
				keywordSearchText = "";
				unloadingForDashBoard();
			}else{
				var html = "<h4 style='color:red'>No Reviews Found</h4>";
				$('#keywordInnerPageSummaryReviews').append(html);
			}
			unloadingForDashBoard();
		}else{
			unloadingForDashBoard();
			$('#wrapper').mask(response.errorMessage);
		}
...
function listKeywordInnerPageReviewsResponse(response,keywordSearchTextVal){
	var tempHtml = "";
	var list = response.successObject.reviewsForInnerPage;
	var departmentName = $('#innerPageKeywordSummaryKeywordName').text();
	for(var i=0;i<list.length;i++){
		...
		var kpiTagSentimentAnalysisUIList = list[i].kpiTagSentimentAnalysisUIList;
		tempHtml+=	'<div class="col-xs-12 SingleReviewList">'+
		'<div class="col-xs-12 col-sm-3 col-lg-2 LightBlue ShowSemanticPolarity" onclick="expandKpiFactorsForKeywordInnerPage('+id+')">'+
		'<div class="PositiveSentimentReview row"><div class="arrow-left"></div>Review Score:'+repufactorScore+'</div>'+
			'<div class="reviewDetails row">'+
			'<div class="reviewSource">'+sourceName+'</div>'+
				'<div class="reviewerName">by <span>'+reviewerName+'</span></div>'+
				'<div class="reviewerDetail">from <span>'+reviewLocation+'</span></div>'+
				'<div class="revieweTime"><span class="glyphicon glyphicon-time"></span>'+
					'<span>'+$.datepicker.formatDate('d M yy',new Date(reviewTime))+'</span>'+
				'</div>'+
			'</div>'+
		'</div>'+
		'<div class="col-xs-12 col-sm-9 col-lg-10">';
		if(keywordSearchTextVal!=""){
		tempHtml+='<h3 class="SingleReviewHeader">'+highlightSearchText(keywordSearchTextVal, reviewTitle)+'</h3>'+
			'<p>'+highlightSearchText(keywordSearchTextVal, reviewContent)+'</p>';
		}else{
			tempHtml+=	'<h3 class="SingleReviewHeader">'+reviewTitle+'</h3>'+
						'<p>'+reviewContent+'</p>';
		}
			
		tempHtml+='<div class="LightGreyColor col-xs-12 sentimentKeywordResult">'+
				'<div class="VerySmallGreyContent">This review was rated <b>'+normalizedRating+'/'+totalRating+'</b> for <b>'+departmentName+'</b> by <b>'+sourceName+'</b></div>'+
			'</div>';
		
		tempHtml+='<div  class="TradeReviewKpiDepartmentFactor col-xs-12 OnSeleceActive" id="keywordInnerPageKpiScores_'+id+'" style="margin:0px;display:none;">';
		tempHtml+='<div class="KPIScoreHeader SmallBoldGreyContent col-xs-12">Keywords</div>';
		console.log(kpiTagSentimentAnalysisUIList);
		for(var j=0;j<kpiTagSentimentAnalysisUIList.length;j++){
			var kpiName = kpiTagSentimentAnalysisUIList[j].nlpQueryName;
			var score = parseFloat(kpiTagSentimentAnalysisUIList[j].kpiFactorScore);
			score = score.toFixed(2);
			tempHtml+=	'<div class="KPIScore col-xs-4">'+score+'<span class="PositiveSentimentCount">&nbsp;'+score+'%</span></div>';

		}	
		
		tempHtml+= '</div>';
		tempHtml+=	'<div id="keywordInnerPageViewDetails_'+id+'" style="margin-left: -169px;"></div>';
		tempHtml+=	'</div></div>'+
	'</div>';
	}
	 var currentPage = response.successObject.paginationDto.currentPage;
	 tempHtml += '<input type="hidden" value="'+currentPage+'" id="currentPage">';
	 var noOfPages = response.successObject.paginationDto.noOfPages;
	 var beginPage = response.successObject.paginationDto.beginPage;
	 var endPage = response.successObject.paginationDto.endPage;
	 var recordsPerPage = response.successObject.paginationDto.recordsPerPage;
	 
	 tempHtml+=			'<ul class="pagination" style="margin-left:25px;">';
	 if(beginPage-1 == 0 ){
		// tempHtml+=			'<li><a href="#" disable="disabled">Prev</a></li>';
	 }else{
		 tempHtml+=			'<li><a href="#" onclick="listKeywordInnerPageReviews('+(beginPage-1)+');">&laquo;</a></a></li>';
	 }
	 for(var i=beginPage;i <= endPage;i++){
		 if(i == currentPage){
			 tempHtml+=			'<li class="active"><a href="#" onclick="listKeywordInnerPageReviews('+(beginPage)+');">'+(beginPage)+'</a></li>';
		 }else{
			 tempHtml+=			'<li><a href="#" onclick="listKeywordInnerPageReviews('+(beginPage)+');">'+(beginPage)+'</a></li>';
		 }
		 beginPage++;
	 }
	 if(endPage >= noOfPages){
	 }else{
		  tempHtml+= 		'<li><a href="#" onclick="listKeywordInnerPageReviews('+(endPage+1)+');">&raquo;</a></li>';
	 }
	
	 if(currentPage <= 0 && list.length==0){
		tempHtml+=	'<p style="color:#B94A48;">No Records Found</p>';
	 }else{
	 }
	 tempHtml+= 		'</ul>';
	 return tempHtml;
}
function expandKpiFactorsForKeywordInnerPage(reviewId){
	$('#keywordInnerPageKpiScores_'+reviewId).show(600);
}
...
$("#applyFilterBtn").click(function(e){
	var organizationId = $('#mySelect option:selected').val();
...
function getReviewsPerPage(begin, end, polarity){
	...
	for(var i=0;i<sublist.length;i++){
		...
		$("#keywordInnerPageSummaryReviews").append(html).show();
		$('span.stars').stars();
		}}
	$('.ShowSemanticPolarity').click(function(){
		var keywordDivId="keywordAndScore_"+$(this).data('reviewid');
		...
		$.ajax({
			...
			success : function(response) {
				if (response.status == "SAVE_SUCCESS") {
					$('#keywordInnerPageSuccessModalTitle').text("Success");
					$('#keywordInnerPageSuccessModalText').text("Respond successfully has been mailed !");
					$('#keywordInnerPageSuccessModal').modal('show');
				}else{
					if (response.status == "SAVE_ERROR") {
						$('#keywordInnerPageSuccessModalTitle').text("Error");
						$('#keywordInnerPageSuccessModalText').text(response.errorMessage);
						$('#keywordInnerPageSuccessModal').modal('show');
					}
				}
			},
			error : function(response) {
				$('#keywordInnerPageSuccessModalTitle').text("Error");
				$('#keywordInnerPageSuccessModalText').text("something went wrong please contact admin");
				$('#keywordInnerPageSuccessModal').modal('show');
			}
		});
	}
...
/*function saveFlag(reviewId) {
	...
	if (anyChecked == false) {
		$('#keywordInnerPageSuccessModalTitle').text("Alert");
		$('#keywordInnerPageSuccessModalText').text("Please Select one reason");
		$('#keywordInnerPageSuccessModal').modal('show');
		return false;
	}
	...
	$.ajax({
		...
		success : function(response) {
			if (response.status == "SAVE_SUCCESS") {
				$('#keywordInnerPageSuccessModalTitle').text("Success");
				$('#keywordInnerPageSuccessModalText').text("Flag Sent Successfully");
				$('#keywordInnerPageSuccessModal').modal('show');
...
function saveShare(reviewId) {
	...
	$.ajax({
		...
		success : function(response) {
			if (response.status == "SAVE_SUCCESS") {
				$('#keywordInnerPageSuccessModalTitle').text("Success");
				$('#keywordInnerPageSuccessModalText').text("Posted Successfully !");
				$('#keywordInnerPageSuccessModal').modal('show');
			}else{
				if(response.status == "SAVE_ERROR"){
					$('#keywordInnerPageSuccessModalTitle').text("Error");
		...
		error : function(response) {
			$('#keywordInnerPageSuccessModalTitle').text("Error");
			$('#keywordInnerPageSuccessModalText').text("something went wrong");
			$('#keywordInnerPageSuccessModal').modal('show');
		}
	});
}
...
function saveBroadcast(reviewId) {
	...
	$.ajax({
		...
		success : function(response) {
			if (response.status == "SAVE_SUCCESS") {
				$('#keywordInnerPageSuccessModalTitle').text("Success");
				$('#keywordInnerPageSuccessModalText').text("Broadcast Status Set Successfully !");
				$('#keywordInnerPageSuccessModal').modal('show');
			}else{
				$('#keywordInnerPageSuccessModalTitle').text("Success");
				$('#keywordInnerPageSuccessModalText').text("Broadcast Status Set Successfully !");
...
function getNotes(reviewSiteContentId, organizationId) {
	...
	$('a#keywordViewDetails_'+reviewSiteContentId).hide();
	$('a#keywordHideDetails_'+reviewSiteContentId).show();
}
...
$('#mySelect').on('change',function() {
	var orgId = $('#mySelect option:selected').val();
	$.ajax({
				type : "POST",
...
function saveNoteForNotify(reviewId, obj) {
	...
	if (noteVale == "" || dateTimeValue == "" || departmentOptionValue == null
		...
		$('#keywordInnerPageSuccessModalTitle').text("Alert");
		$('#keywordInnerPageSuccessModalText').text("Mandatory fields(*) are required !");
		$('#keywordInnerPageSuccessModal').modal('show');
		return;
	}
	reviewAction = {
	...
	$.ajax({
		...
		error : function(response) {
			$('#keywordInnerPageSuccessModalTitle').text("Alert");
			$('#keywordInnerPageSuccessModalText').text("Something went wrong contact admin !");
			$('#keywordInnerPageSuccessModal').modal('show');
		}
	});
}
		...
		success : function(response) {
			if(response.status=="LIST_SUCCESS"){
				...
				htmlCode+='</div>';
					...
					restoreHotelReviewsDivIdHtml = $('#keywordInnerPageSummaryReviews').html();
					restoreFilterBarHtml = $('#filterBar').html();
					$('#keywordInnerPageSummaryReviews').html(htmlCode);
					var _keywordName=$('#innerPageKeywordSummaryKeywordName').text();
					...
					var barHtmlCode = '<div class="row">'
							...
							+ '<span> <a onclick="restores()">'+_keywordName+'</a></span>'
							+ '<span class="glyphicon glyphicon-chevron-right TineyGreyContent" aria-hidden="true"></span>'
							+ '<span>Keyword Detail</span>' + '</div>' + '</div>';
					$('#keywordInnerPageBreadCrumb').hide();
					$('#keywordInnerPageSummaryReviews').prepend(barHtmlCode);
					$('#keywordLineSeparator').hide();
					$('#keywordInnerFactorTop').hide();
					$('#keywordReviewsSearch').hide();
					$('#keywordInnerPageSummaryReviews').show(600);
						if($("span.stars")!==undefined){
								$('span.stars').stars();
							}
							if($("span.starsTA")!==undefined){
...
function hideKeywordDetails(id){
	$('#keywordInnerPageViewDetails_'+id).hide(600);
	$('#keywordHideDetails_'+id).hide();
	$('#keywordViewDetails_'+id).show();
}
/********************** End Of Nitesh******************/
		...
		success : function(response) {
			if(response.status=="LIST_SUCCESS"){
				...
				htmlCode+='</div>';
					...
					htmlCode += '<label class="col-xs-3 control-label">For<span class="mandatoryField">*</span></label>';
					htmlCode += '<div class="col-xs-4">';
					htmlCode += '<select onchange="populateEmployeesForTasks(this)" id="departmentForTask_'
...
function getMappedSourcesForReviews(reviewId) {
	...
	function restores() {
		$("#notificationsBredcrumb").hide();
		$("#searchBar").show();
		$("#notificationsBredcrumb").remove();
		$('#filterBar').html(restoreFilterBarHtml);
		$('#keywordInnerPageSummaryReviews').hide();
		$('#keywordInnerPageSummaryReviews').html(restoreHotelReviewsDivIdHtml);
		$("#page-selection").show();
		$('#keywordInnerPageBreadCrumb').show();
		$('#keywordLineSeparator').show();
		$('#keywordInnerFactorTop').show();
		$('#keywordReviewsSearch').show();
		$('#reviewsBredcrumb').hide();
		$('#keywordInnerPageSummaryReviews').show();
        $('span.stars').stars();
