	 ...
	 $.ajax({
			...
			unloadingForDashBoard();
				if(response.length>0){
					...
					$("#paginationAdminReviewActionDiv").html(" ");
					$("#adminReviewActions").html("<h4><font color='red'>No New Reviews Found!!</font></h4>").show();
				}
...
function getReviewsPerPage(begin, end){
	...
	for(var i=begin;i<end;i++){
		response.push(actionWithReviewDetailResponse[i]);
					...
					$('#adminReviewActions').html('');
					for(var i=0;i<response.length;i++){
						var reviewDetailForAction = response[i].review;console.log(reviewDetailForAction);
						var tempHtml = "";
						...
						tempHtml += '<div class="reviewSource">' + reviewDetailForAction.sourceName + '</div>';
							...
							if(reviewDetailForAction.reviewerName==null || $.trim(reviewDetailForAction.reviewerName)==""){
						...
						tempHtml += '</div>';
						tempHtml +='</div>';
						$('#adminReviewActions').append(tempHtml).show();
					}
				}
		...
		success : function(response) {
			if(response.status=="LIST_SUCCESS"){
				...
				/*	htmlCode += '<li class="">';
					...
					restoreHotelReviewsDivIdHtml = $('#adminReviewActions').html();
					restoreFilterBarHtml = $('#filterBar').html();
					

					$('#adminReviewActions').html(htmlCode);
					...
					var barHtmlCode = '<div class="row">'
							...
							+ '<span> <a onclick="restores()">Admin Review Actions </a></span>'
							+ '<span class="glyphicon glyphicon-chevron-right TineyGreyContent" aria-hidden="true"></span>'
							+ '<span>Admin Review Actions Datails</span>' + '</div>' + '</div>';

					/*$('#filterBar').html(barHtmlCode);*/
					$('#adminReviewActions').prepend(barHtmlCode);
					$('#adminReviewActions').show(600);
				...
				console.log(reviewForViewDetail);
				   var htmlCode = "";
					...
					htmlCode += '<div class="reviewSource">' + reviewForViewDetail.sourceName + '</div>';
						...
						if(reviewForViewDetail.reviewerName==null || $.trim(reviewForViewDetail.reviewerName)==""){
					...
					if(reviewForViewDetail.reviewLocation==null || reviewForViewDetail.reviewLocation==""){	
						htmlCode += '<div class="reviewerDetail"><span>';
						htmlCode += '</span></div>';
					}
					else{
						htmlCode += '<div class="reviewerDetail">from <span>';
		...
		$.ajax({
			...
			success : function(response) {
				if (response.status == "SAVE_SUCCESS") {
					$('#notificationReviewNotificationsModal').text("Success");
					$('#notificationReviewNotificationsModalText').text("Respond successfully has been mailed !");
					$('#notificationReviewNotificationsModalModal').modal('show');
					$('#respond').val('');
				}else{
					if (response.status == "SAVE_ERROR") {
						$('#notificationReviewNotificationsModal').text("Success");
						$('#notificationReviewNotificationsModalText').text(response.errorMessage);
						$('#notificationReviewNotificationsModalModal').modal('show');
					}
				}
			},
			error : function(response) {
				$('#notificationReviewNotificationsModal').text("Success");
				$('#notificationReviewNotificationsModalText').text("something went wrong");
				$('#notificationReviewNotificationsModalModal').modal('show');
			}
		});
	}
...
function saveFlag(reviewId) {
	...
	if (anyChecked == false) {
		$('#notificationReviewNotificationsModal').text("Success");
		$('#notificationReviewNotificationsModalText').text("Please Select one reason");
		$('#notificationReviewNotificationsModalModal').modal('show');
		return false;
	}
	...
	$.ajax({
		...
		success : function(response) {
			if (response.status == "SAVE_SUCCESS") {
				$('#notificationReviewNotificationsModal').text("Success");
				$('#notificationReviewNotificationsModalText').text("Flag Sent Successfully");
				$('#notificationReviewNotificationsModalModal').modal('show');
		...
		error : function(response) {
			$('#notificationReviewNotificationsModal').text("Error");
			$('#notificationReviewNotificationsModalText').text(response.status);
			$('#notificationReviewNotificationsModalModal').modal('show');
		}
	});
	return false;
...
function restores() {
	...
	$('#adminReviewActions').hide();
	$('#adminReviewActions').html(restoreHotelReviewsDivIdHtml);
	$("#notificationsBredcrumb").remove();
	$("#paginationAdminReviewActionDiv").show();
	var barHtmlCode = '<div class="row" id="notificationsBredcrumb">'
		+ '<div class="col-lg-12 SubHeading SmallDarkGreyHeader"><span> <a href="../notifications/showNotifications.htm">Notifications</a></span>'
		+ ' <span class="glyphicon glyphicon-chevron-right SmallGreyContent" aria-hidden="true"></span><span>Admin Review Actions </span></div>' + '</div>';
	$('#reviewActionsBredcrumb').hide();
	$('#adminReviewActions').prepend(barHtmlCode).show();
	$('#adminReviewActions').show();
...
/*function saveFlag(reviewId) {
	...
	if (anyChecked == false) {
		$('#notificationReviewNotificationsModal').text("Alert");
		$('#notificationReviewNotificationsModalText').text("Please Select one reason");
		$('#notificationReviewNotificationsModalModal').modal('show');
		return false;
	}
...
function saveShare(reviewId) {
	...
	$.ajax({
		...
		success : function(response) {
			if (response.status == "SAVE_SUCCESS") {
				$('#notificationReviewNotificationsModal').text("Success");
				$('#notificationReviewNotificationsModalText').text("Posted Successfully !");
				$('#notificationReviewNotificationsModalModal').modal('show');
...
function saveBroadcast(reviewId) {
	...
	$.ajax({
		...
		error : function(response) {
			$('#notificationReviewNotificationsModal').text("Error");
			$('#notificationReviewNotificationsModalText').text("something went wrong Please contact admin");
			$('#notificationReviewNotificationsModalModal').modal('show');
		}
	});
}
