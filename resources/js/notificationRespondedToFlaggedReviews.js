...
loadingForDashBoard();
	$.ajax({
		type:"POST",
		url:"../notifications/getResponedFlaggedReviews.htm?organizationId="+organizationId,
	    ...
	    $('#paginationRespondedFlaggedReviewsDiv').bootpag({
	        total: noOfPages,
	        page: 1,
	        maxVisible: 5
		...
		$("#paginationRespondedFlaggedReviewsDiv").html(" ");
...
function getReviewsPerPage(begin, end){
		...
		for(var j=0;j<response.length;j++){
				var repufactorScore=response[j].repufactorScore;
				var reviewRespondedFlagReviewDetails = response[j].review;/*console.log(reviewRespondedFlagReviewDetails);*/
			    var reviewId=response[j].id;
				var reviewTitle=response[j].reviewTitle;/*console.log(reviewTitle);*/
				var sourceName = response[j].sourceName;
				var reviewContent =response[j].highlightedReviewContent;
				var reviewTime=response[j].reviewTime;
				var flagStatus=response[j].flagStatus;
				var reviewLanguage=response[j].reviewLanguage;
				var reviewerName=response[j].reviewerName;
				var reviewOverallRating=response[j].reviewOverallRating;
				var reviewFlagComment=response[j].flagComment;
				var reviewFlagApproved=response[j].flagApproved;
				var reviewCreatedDate=response[j].createdDate;
				var reviewerLocation=response[j].reviewLocation;
				var sentimentClass;
				
			
				for (var p = 0; p < sentimentPolarityList.length; p++) {
					if (Math.round(repufactorScore) >= sentimentPolarityList[p].minPercentage
						&& Math.round(repufactorScore) <= sentimentPolarityList[p].maxPercentage
					&& sentimentPolarityList[p].sentimentName == "negative") {
					sentimentClass="NegativeSentimentReview row";
					break;
					}
					if (Math.round(repufactorScore) >= sentimentPolarityList[p].minPercentage
							&& Math.round(repufactorScore) <= sentimentPolarityList[p].maxPercentage
						&& sentimentPolarityList[p].sentimentName == "positive") {
						sentimentClass="PositiveSentimentReview row";
						break;
					...
					if (Math.round(repufactorScore) >= sentimentPolarityList[p].minPercentage
						...
						sentimentClass="NeutralSentimentReview row";
						break;
					}
			...
			html='<div class="row col-xs-12 SingleReviewList">'
					+'<div data-reviewid="'+response[j].id+'" class="col-xs-12 col-sm-3 col-lg-2 LightBlue ShowSemanticPolarity">'
						+'<div  class="'+sentimentClass+'"><div class="arrow-left"></div>Review Score: <span class="score">'+repufactorScore.toFixed(2)+'% </span> </div>'
						+'<div class="reviewDetails row">'
							+'<div class="reviewSource">'+sourceName+'</div>'
							+'<div class="reviewerName">by <span>'+reviewerName+'</span></div>'
							+'<div class="reviewerDetail">from <span>'+reviewerLocation+'</span></div>'
					...
					+'</div>'//end ShowSemanticPolarity  
					+'<div class="col-xs-12 col-sm-9 col-lg-10">'
					      +'<div style="float:right;">'
					        + '<input type="button" class="btn btn-ViewReviewDetails" onclick="viewDetail('+ reviewId + ')" /> '
					      +'</div>'
					+ '<div class="col-xs-12 col-sm-9 col-lg-10">';
					...
					html +='<p>'+reviewContent+'</p>'
						...
						+'<div class="SubHeading col-xs-12">'
							+'<div class="row" style="border:none;">'
								...
								if(reviewFlagApproved==1){
									html +='<div><img src="../resources/images/correct.png"><font color="green">Your request has been Approved.This review has been Deleted from Repufact.</font>';
									html +='</div>';
								}else if(reviewFlagComment!=null && reviewFlagComment!="" && reviewFlagComment!=undefined){
									html +='<div><img src="../resources/images/wrong.jpg"><font color="red">Your request has been rejected due to : </font>'+reviewFlagComment;
									html +='</div>';
								}
								     html+='<div class="VerySmallBoldGreyContent marginRight20 float-left">Reason For Flagging :'
								    	  +'</div>'
								    	  +'<div class="float-left" id="">'+flagStatus
								    	  +'</div>'
								+'</div>'
								+'<div class="col-xs-12"><div class="revieweTime"><span class="glyphicon glyphicon-time"> </span><span>'+moment(reviewCreatedDate).format("DD MMMM YYYY")+'</span>'
								+'</div></div>'
				           +'</div>'
				        +'</div>'
		...
		$('.ShowSemanticPolarity').click(function() {
			...
			/*console.log(keywordDivId+" 121212");*/
...
/*	function viewDetail(reviewId) {
		$('#paginationRespondedFlaggedReviewsDiv').hide();
		$('#notificationRespondedDivHeading').hide();
		$('#reviewActionsBredcrumb').html(" ");
		var actionDetails = '';
		for(var i=0;i<actionWithReviewDetailResponse.length;i++){console.log(actionWithReviewDetailResponse);
				...
				if(response.status=="LIST_SUCCESS"){
					...
					console.log(reviewForViewDetail);
					   var htmlCode = "";
						...
						htmlCode +='<span class="glyphicon glyphicon-bell"></span>'+moment(actionDetails.completionDate).format("ddd, DD MMM YYYY h:mm:ss a")+'</span>';
						htmlCode += '</div>';
						...
						if(reviewForViewDetail.keywordList.length>0){
							htmlCode += '<div id="keywordAndScore_'+reviewForViewDetail.id+'" class="TradeReviewKpiDepartmentFactor col-xs-12">'
						...
						restoreHotelReviewsDivIdHtml = $('#responseReviewFlaggedDiv').html();
						restoreFilterBarHtml = $('#filterBar').html();
						

						$('#responseReviewFlaggedDiv').html(htmlCode);
						...
						var barHtmlCode = '<div class="row">'
								...
								+ '<span> <a onclick="restores()"> Flagged </a></span>'
								+ '<span class="glyphicon glyphicon-chevron-right TineyGreyContent" aria-hidden="true"></span>'
								+ '<span>Flagged Review Datails</span>' + '</div>' + '</div>';

						$('#filterBar').html(barHtmlCode);
						$('#responseReviewFlaggedDiv').prepend(barHtmlCode);
						$('#responseReviewFlaggedDiv').show(600);
					...
					reviewForViewDetail=response.successObject.review;
					   ...
					   var htmlCode = "";
						...
						for (var h = 0; h < reviewForViewDetail.kpiIndustryMasterUiList.length; h++) {
							...
							htmlCode +='<div class="SubHeading col-xs-12">';
							htmlCode +='<div class="row ActionReports ReviewActionIcon" style="border:none;">';
							htmlCode +='<div class="col-xs-5 SmallBoldGreyContent">Flag</div>';
							htmlCode +='<div class="col-xs-7 text-right SmallDarkGreyHeader">Completion by <span class="SmallRedHeader">';
							htmlCode +='<span class="glyphicon glyphicon-bell"></span>'+moment(actionDetails.completionDate).format("ddd, DD MMM YYYY h:mm:ss a")+'</span>';
							htmlCode += '</div>';
							htmlCode +='<div class="col-xs-12">'+actionDetails.flagComment+'</div>';
							htmlCode +='<div class="col-xs-12 SmallDarkGreyHeader">by <span class="VerySmallBoldGreyContent marginRight20">'+actionDetails.commentedBy+' </span>';
							htmlCode +='<span>For <span class="VerySmallBoldGreyContent">'+actionDetails.commentedFor+' </span>at <span class="VerySmallBoldGreyContent">'+actionDetails.departmentName+'</span>';
							htmlCode +='</span></div>';
							htmlCode +='<div class="col-xs-12">';
...
*/							
							...
							+'<div class="row" style="border:none;">'
								...
								+'</div>'
								+'<div class="col-xs-12 SmallDarkGreyHeader">';
								if(actionDetails.reviewFlagApproved==1){
									htmlCode +='<div><img src="../resources/images/correct.png"><font color="green">Your request has been Approved.This review has been Deleted from Repufact.</font>';
									htmlCode +='</div>';
								}else if(actionDetails.flagComment!=null && actionDetails.flagComment!="" && actionDetails.flagComment!=undefined){
									htmlCode +='<div><img src="../resources/images/wrong.jpg"><font color="red">Your request has been rejected due to : </font>'+actionDetails.flagComment;
									htmlCode +='</div>';
								}
								htmlCode+='<div class="VerySmallBoldGreyContent marginRight20 float-left">Reason For Flagging :'
								    	  +'</div>'
								    	  +'<div class="float-left" id="">'+actionDetails.flagStatus
								    	  +'</div>'
								+'</div>'
								+'<div class="col-xs-12"><div class="revieweTime"><span class="glyphicon glyphicon-time"> </span><span>'+moment(actionDetails.reviewCreatedDate).format("DD MMMM YYYY")+'</span>'
								+'</div></div>'
				           +'</div>'
				        +'</div>';
	...
	function saveFlags(reviewId) {
		...
		if (anyChecked == false) {
			$('#notificationRespondedFlagsSuccessModalTitle').text("Alert");
			$('#notificationRespondedFlagsSuccessModalText').text("Please Select one reason");
			$('#notificationRespondedFlagsSuccessModal').modal('show');
		...
		$.ajax({
			...
			success : function(response) {
				if (response.status == "SAVE_SUCCESS") {
					$('#notificationRespondedFlagsSuccessModalTitle').text("Success");
					$('#notificationRespondedFlagsSuccessModalText').text("Flagged Successfully");
					$('#notificationRespondedFlagsSuccessModal').modal('show');
	...
	function restores() {
		...
		$('#responseReviewFlaggedDiv').hide();
		$('#responseReviewFlaggedDiv').html(restoreHotelReviewsDivIdHtml);
		$("#notificationsBredcrumb").remove();
		$("#page-selection").show();
		var barHtmlCode = '<div class="row" id="notificationsBredcrumb">'
			+ '<div class="col-lg-12 SubHeading SmallDarkGreyHeader"><span> <a href="../notifications/showNotifications.htm">Notifications</a></span>'
			+ ' <span class="glyphicon glyphicon-chevron-right SmallGreyContent" aria-hidden="true"></span><span> Flagged </span></div>' + '</div>';
		$('#reviewActionsBredcrumb').hide();
		$('#responseReviewFlaggedDiv').prepend(barHtmlCode).show();
		$('#responseReviewFlaggedDiv').show();
	...
	function saveShares(reviewId) {
		...
		$.ajax({
			...
			success : function(response) {
				if (response.status == "SAVE_SUCCESS") {
					$('#notificationRespondedFlagsSuccessModalTitle').text("Alert");
					$('#notificationRespondedFlagsSuccessModalText').text("Posted Successfully !");
					$('#notificationRespondedFlagsSuccessModal').modal('show');
					document.getElementById("shareCountSpan" + reviewId).innerHTML = 'Share'
						+ '(' + reviewSourceMapping.length + ')';
				}else{
					if(response.status == "SAVE_ERROR"){
						$('#notificationRespondedFlagsSuccessModalTitle').text("Error");
						$('#notificationRespondedFlagsSuccessModalText').text("Some Thing went wrong Please Contact Admin");
						$('#notificationRespondedFlagsSuccessModal').modal('show');
					}
				}
			}, 
	...
	function saveBroadcasts(reviewId) {
		...
		$.ajax({
			...
			success : function(response) {
				if (response.status == "SAVE_SUCCESS") {
					$('#notificationRespondedFlagsSuccessModalTitle').text("Success");
					$('#notificationRespondedFlagsSuccessModalText').text("Review Broadcasted Successfully.");
					$('#notificationRespondedFlagsSuccessModal').modal('show');
				}else{
					$('#notificationRespondedFlagsSuccessModalTitle').text("Error");
					$('#notificationRespondedFlagsSuccessModalText').text("Some Thing went wrong Please Contact Admin");
...
/*	function getNotes(reviewSiteContentId, organizationId) {
	...
	/*function saveNoteForTasks(reviewId, obj) {
		...
		if (noteVale == "" || dateTimeValue == "" || departmentOptionValue == null
			...
			$('#notificationRespondedFlagsSuccessModalTitle').text("Alert");
			$('#notificationRespondedFlagsSuccessModalText').text("Mandatory fileds(*) are required !");
			$('#notificationRespondedFlagsSuccessModal').modal('show');
			return;
		}
		...
		$.ajax({
			...
			error : function(response) {
				$('#notificationRespondedFlagsSuccessModalTitle').text("Error");
				$('#notificationRespondedFlagsSuccessModalText').text("Something went wrong , please contact admin !");
				$('#notificationRespondedFlagsSuccessModal').modal('show');
			}
		});
	}
