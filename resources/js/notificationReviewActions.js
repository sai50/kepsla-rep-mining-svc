	 ...
	 $.ajax({
			...
			success:function(response){
				if(response.length>0){
				    ...
				    $('#reviewActionsPaginationDiv').bootpag({
				        total: noOfPages,
				        page: 1,
				        maxVisible: 5
					...
					$("#notificationReviews").html("<h4><font color='red'>No New Reviews Found!!</font></h4>").show();
				}
	    ...
	    var response = [];
	     for(var i=begin;i<end;i++){
		  response.push(actionWithReviewDetailResponse[i]);
                  ...
                  $('#notificationReviewActions').html('');
					for(var i=0;i<response.length;i++){console.log(response);
						...
						tempHtml += '<div class="col-xs-12 col-sm-9 col-lg-10">';	
							...
							tempHtml +='<div class="SubHeading col-xs-12">';
								tempHtml +='<div class="row SubHeading ActionReports ReviewActionIcon" style="border:none;">';
									tempHtml +='<div class="col-xs-5 SmallBoldGreyContent">Task : '+response[i].actionType+'</div>';
									tempHtml +='<div class="col-xs-7 text-right SmallDarkGreyHeader">Completion by <span class="SmallRedHeader">';
											tempHtml +='<span class="glyphicon glyphicon-time"></span>'+response[i].completionDate+'</span>';
									tempHtml += '</div>';
									tempHtml +='<div class="col-xs-12">'+response[i].comment+'</div>';
...
/*function viewDetail(reviewId) {
	$('#reviewActionsPaginationDiv').hide();
			...
			if(response.status=="LIST_SUCCESS"){
				...
				console.log(reviewForViewDetail);
				   var htmlCode = "";
					...
					htmlCode +='<div class="col-xs-5 SmallBoldGreyContent">Task : '+actionDetails.actionType+'</div>';
					...
					htmlCode += '</div>';
					htmlCode +='<div class="col-xs-12">'+actionDetails.comment+'</div>';
				...
				htmlCode+='</div>';
					...
					htmlCode += '<input id="datetimepickerForTask_'
							+ reviewForViewDetail.id
							+ '" onmouseover="activateDatetimepickers(this)" class="form-control input-sm" placeholder="Select Date and time">';
					htmlCode += '</div>';
					htmlCode += '</div>';
					htmlCode += '<div class="form-group input-group form-inline col-xs-12">';
					...
					restoreHotelReviewsDivIdHtml = $('#notificationReviewActions').html();
					restoreFilterBarHtml = $('#filterBar').html();
					

					$('#notificationReviewActions').html(htmlCode);
					...
					$('#notificationReviewActions').prepend(barHtmlCode);
					$('#notificationReviewActions').show(600);
...
function restores() {
	...
	$('#notificationReviewActions').hide();
	$('#notificationReviewActions').html(restoreHotelReviewsDivIdHtml);
	$("#notificationsBredcrumb").remove();
	$("#reviewActionsPaginationDiv").show();
	...
	$('#notificationReviewActions').prepend(barHtmlCode).show();
	$('#notificationReviewActions').show();
...
function openShare(reviewId){
	getMappedSourcesForReviews(reviewId);
	document.getElementById('Share-pills'+reviewId).setAttribute( "class", "row SubHeading tab-pane active" );
}
function closeNote(reviewId){
	document.getElementById('Note-pills'+reviewId).setAttribute( "class", "row SubHeading tab-pane fade" );
...
function populateEmployeesForTasks(obj) {
	var organizationId = $('#organizationName option:selected').val();
...
/*function saveNoteForTicket(reviewId, obj) {
	...
	if (noteVale == "" || dateTimeValue == "" || departmentOptionValue == null
		...
		alert("Mandatory fileds(*) are required !");
		return;
	}
