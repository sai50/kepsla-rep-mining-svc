...
var noteWithReviewDetailResponse;
var departmentList = [];
...
function getNotificationReviewNotes(selectedOrgId){
	$('#notificationReviewNotesBredcrumb').show();
	loadingForDashBoard();
	 $.ajax({
			type:"GET",
			...
			success:function(response){
				...
				if(response.length>0){
					noteWithReviewDetailResponse = response;
					var noOfPages = 0;
				    ...
				    $('#reviewNotesPaginationDiv').bootpag({
				        total: noOfPages,
				        page: 1,
				        maxVisible: 5
					...
					$("#reviewActionsPaginationDiv").html(" ");
					$("#notificationReviewNotes").html("<h4><font color='red'>No New Reviews Found!!</font></h4>").show();
				}
...
function getReviewsPerPage(begin, end){
	...
	for(var i=begin;i<end;i++){
		response.push(noteWithReviewDetailResponse[i]);
	      }
					$('#notificationReviewNotes').html('');
					for(var i=0;i<response.length;i++){
						if(response[i]!=undefined){
						var html = "";
						console.log(reviewDetailForNote);
						var reviewDetailForNote = response[i].review;
						var tempHtml = "";
						tempHtml +='<div class="row col-xs-12 SingleReviewList">';
						tempHtml +='<div data-reviewid="'+reviewDetailForNote.id+'" class="col-xs-12 col-sm-3 col-lg-2 LightBlue ShowSemanticPolarity">';
						for (var p = 0; p < sentimentPolarityList.length; p++) {
							if (parseInt(reviewDetailForNote.repufactorScore) >= sentimentPolarityList[p].minPercentage
									&& parseInt(reviewDetailForNote.repufactorScore) <= sentimentPolarityList[p].maxPercentage
								...
								tempHtml += '<div class="PositiveSentimentReview row"><div class="arrow-left"></div>Review Score: <span class="score">'
										+ reviewDetailForNote.repufactorScore.toFixed(1)
										+ '%</span> </div>';
								break;
							}
							if (parseInt(reviewDetailForNote.repufactorScore) >= sentimentPolarityList[p].minPercentage
						...
						tempHtml += '<div class="reviewSource">' + reviewDetailForNote.sourceName + '</div>';
						
							if(reviewDetailForNote.reviewerName==null || $.trim(reviewDetailForNote.reviewerName)==""){
								tempHtml += '<div class="reviewerName"><span>';
								tempHtml+='';
							}else{
								tempHtml += '<div class="reviewerName">by <span>';
								tempHtml+= ''+reviewDetailForNote.reviewerName + '';
								
							}
						tempHtml+='</span></div>';
								
						
						if(reviewDetailForNote.reviewLocation==null || reviewDetailForNote.reviewLocation==""){	
							tempHtml += '<div class="reviewerDetail"><span>';
							tempHtml += '</span></div>';
						}
						else{
							tempHtml += '<div class="reviewerDetail">from <span>';
							tempHtml += ''+reviewDetailForNote.reviewLocation + '</span></div>';
						}
						tempHtml += '<div class="revieweTime"><span class="glyphicon glyphicon-time"> '
								+ moment(reviewDetailForNote.reviewTime).format("DD MMMM YYYY")
								+ '</span>';
						tempHtml += '</div>';
						...
						tempHtml += '<input type="button" class="btn btn-ViewReviewDetails" onclick="viewDetail('+ reviewDetailForNote.id + ')" /> ';
						tempHtml+='</div>';
						tempHtml += '<div class="col-xs-12 col-sm-9 col-lg-10">';	
						if (reviewDetailForNote.reviewTitle != null) {
							tempHtml += '<h3 class="SingleReviewHeader" >'
									+ reviewDetailForNote.reviewTitle + '</h3>';
						}
						tempHtml += '<p>' + reviewDetailForNote.highlightedReviewContent + '</p>';
						
						
						if(reviewDetailForNote.keywordList.length>0){
							tempHtml += '<div id="keywordAndScore_'+reviewDetailForNote.id+'" class="TradeReviewKpiDepartmentFactor col-xs-12">'
									+ '<div class="KPIScoreHeader SmallBoldGreyContent col-xs-12">Keywords</div>';
							for (var h = 0; h < reviewDetailForNote.keywordList.length; h++) {
								for (var p = 0; p < sentimentPolarityList.length; p++) {
									if (parseInt(reviewDetailForNote.keywordList[h].sentimentScore) >= sentimentPolarityList[p].minPercentage
											&& parseInt(reviewDetailForNote.keywordList[h].sentimentScore) <= sentimentPolarityList[p].maxPercentage
											&& sentimentPolarityList[p].sentimentName == "positive") {
										tempHtml +='<div class="KPIScore col-xs-4"> <span class="PositiveSentimentCount"> '+reviewDetailForNote.keywordList[h].nlpQueryName+'</span></div>';
										break;
									}
									if (parseInt(reviewDetailForNote.keywordList[h].sentimentScore) >= sentimentPolarityList[p].minPercentage
											&& parseInt(reviewDetailForNote.keywordList[h].sentimentScore) <= sentimentPolarityList[p].maxPercentage
											&& sentimentPolarityList[p].sentimentName == "neutral") {
										tempHtml +='<div class="KPIScore col-xs-4"> <span class="NeutralSentimentCount"> '+reviewDetailForNote.keywordList[h].nlpQueryName+'</span></div>';
										break;
									}
									if (parseInt(reviewDetailForNote.keywordList[h].sentimentScore) >= sentimentPolarityList[p].minPercentage
											&& parseInt(reviewDetailForNote.keywordList[h].sentimentScore) <= sentimentPolarityList[p].maxPercentage
											&& sentimentPolarityList[p].sentimentName == "negative") {
										tempHtml +='<div class="KPIScore col-xs-4">  <span class="NegativeSentimentCount"> '+reviewDetailForNote.keywordList[h].nlpQueryName+'</span></div>';
										break;
									}
								}
							}				
						...
						tempHtml +='<div class="row ActionReports ReviewActionIcon" style="border:none;">';
						tempHtml +='<div class="col-xs-12">'+response[i].note+'</div><div class="col-xs-12 SmallDarkGreyHeader">by <span class="VerySmallBoldGreyContent marginRight20">'+response[i].userName+' </span>';
						tempHtml +='<span>For <span class="VerySmallBoldGreyContent">'+response[i].externalEmail+' </span>at <span class="VerySmallBoldGreyContent">'+response[i].departmentName+'</span>';
						tempHtml +='</span></div><div class="col-xs-12"><div class="revieweTime"><span class="glyphicon glyphicon-time">'+response[i].createdDate+'</span>';
						tempHtml +='</div></div></div></div>';
						tempHtml += '</div>';
						tempHtml +='</div>';

						$('#notificationReviewNotes').append(tempHtml).show();
					}}
					$('.ShowSemanticPolarity').click(function() {
						var keywordDivId="keywordAndScore_"+$(this).data('reviewid');
						console.log(keywordDivId+" 1111");
...
/*function viewDetail(reviewId) {
	$('#reviewNotesPaginationDiv').hide();
	$('#notificationReviewNotesBredcrumb').html(" ");
	var noteDetails = '';
	for(var i=0;i<noteWithReviewDetailResponse.length;i++){
		if(noteWithReviewDetailResponse[i].review.id==reviewId){
			noteDetails = noteWithReviewDetailResponse[i];console.log(noteDetails);
		}
	}
	$.ajax({
		type : "POST",
		...
		success : function(response) {
			if(response.status=="LIST_SUCCESS"){
				reviewForViewDetail=response.successObject.review;console.log(reviewForViewDetail);
				   var htmlCode = "";
					htmlCode += '<div>';
					htmlCode += '<div class="col-xs-12 SingleReviewList" style="border:none;">';
					htmlCode += '<div class="col-xs-12 col-sm-3 col-lg-2 LightBlue ShowSemanticPolarity">';
					...
					//actions
					htmlCode +='<div class="SubHeading  col-xs-12 col-xs-12">';
					htmlCode +='<div class="row ActionReports ReviewActionIcon" style="border:none;">';
					htmlCode +='<div class="col-xs-5 SmallBoldGreyContent">'+noteDetails.note+'</div>';
					htmlCode +='<div class="col-xs-7 text-right SmallDarkGreyHeader">Completion by <span class="SmallRedHeader">';
					htmlCode +='<span class="glyphicon glyphicon-bell"></span>'+moment(noteDetails.completionDate).format("ddd, DD MMM YYYY h:mm:ss a")+'</span>';
					htmlCode += '</div>';
					htmlCode +='<div class="col-xs-12"></div>';
					htmlCode +='<div class="col-xs-12 SmallDarkGreyHeader">by <span class="VerySmallBoldGreyContent marginRight20">'+noteDetails.userName+'</span>';
					htmlCode +='<span>For <span class="VerySmallBoldGreyContent">'+noteDetails.externalEmail+'</span>at <span class="VerySmallBoldGreyContent">'+noteDetails.departmentName+'</span>';
					htmlCode +='</span></div>';
					htmlCode +='<div class="col-xs-12">';
					htmlCode +='<div class="revieweTime"><span class="glyphicon glyphicon-time">'+noteDetails.createdDate+'</span>';
					htmlCode +='</div>'; 
				...
				htmlCode+='</div>';
					...
					restoreHotelReviewsDivIdHtml = $('#notificationReviewNotes').html();
					restoreFilterBarHtml = $('#filterBar').html();
					

					$('#notificationReviewNotes').html(htmlCode);
					...
					var barHtmlCode = '<div class="row">'
							...
							+ '<span> <a onclick="restores()">Quick Notes</a></span>'
							+ '<span class="glyphicon glyphicon-chevron-right TineyGreyContent" aria-hidden="true"></span>'
							+ '<span>Review Note</span>' + '</div>' + '</div>';

					$('#filterBar').html(barHtmlCode);
					$('#notificationReviewNotes').prepend(barHtmlCode);
					$('#notificationReviewNotes').show(600);
...
/*function saveFlag(reviewId) {
	...
	if (anyChecked == false) {
		...
		$('#notificationNotesSuccessModalTitle').text("Alert");
		$('#notificationNotesSuccessModalText').text("Please Select one reason !");
		$('#notificationNotesSuccessModal').modal('show');
		return false;
	}
	...
	$.ajax({
		...
		success : function(response) {
			if (response.status == "SAVE_SUCCESS") {
				$('#notificationNotesSuccessModalTitle').text("Alert");
				$('#notificationNotesSuccessModalText').text("Flag Sent Successfully!");
				$('#notificationNotesSuccessModal').modal('show');
...
function restores() {
	...
	$('#notificationReviewNotes').hide();
	$('#notificationReviewNotes').html(restoreHotelReviewsDivIdHtml);
	$("#notificationsBredcrumb").remove();
	$("#reviewNotesPaginationDiv").show();
	var barHtmlCode = '<div class="row" id="notificationsBredcrumb">'
		+ '<div class="col-lg-12 SubHeading SmallDarkGreyHeader"><span> <a href="../notifications/showNotifications.htm">Notifications</a></span>'
		+ ' <span class="glyphicon glyphicon-chevron-right TineyGreyContent" aria-hidden="true"></span><span>Quick Notes</span></div>' + '</div>';
	$('#notificationReviewNotesBredcrumb').hide();
	$('#notificationReviewNotes').prepend(barHtmlCode).show();
	$('#notificationReviewNotes').show();
					...
					for (var i = 0; i < commentsList.length; i++) {
						...
						if (commentsList[i].actionType == "Notify") {
							htmlCode += '<div class="row ActionReports ReviewActionIcon">'
									...
									+ '</div>'
									+ '<div class="col-xs-7 text-right SmallDarkGreyHeader">'
									+ 'Completion by <span class="SmallRedHeader"><span class="glyphicon glyphicon-bell"></span>'+commentsList[i].completionDate+''
									+ '</span>'
									+ '</div>'
									+ '<div class="col-xs-12">'
									+ commentsList[i].comment
						...
						if (commentsList[i].actionType == "General") {
							htmlCode += '<div class="row ActionReports ReviewActionIcon">'
									...
									+ '<div class="col-xs-7 text-right SmallDarkGreyHeader">'
									+ 'Completion by <span class="SmallRedHeader"><span class="glyphicon glyphicon-bell"></span>'
									+ (commentsList[i].completionDate).format("ddd, DD MMM YYYY h:mm:ss a")
									+ '</span>'
									+ '</div>'
									+ '<div class="col-xs-12">'
									+ commentsList[i].comment
...
console.log(noteWithReviewDetailResponse);
		for (var i = 0; i < noteWithReviewDetailResponse.length; i++) {
			if (noteWithReviewDetailResponse.id == reviewId) {
				reviewContent=noteWithReviewDetailResponse.highlightedReviewContent;
				break;
			}
		}
	} else {
		for (var i = 0; i < noteWithReviewDetailResponse.length; i++) {
			if (noteWithReviewDetailResponse.id == reviewId) {
				reviewContent=filteredReviewsDuplicateResponseObject.highlightedReviewContent;
				break;
			}
		}
	}
	...
	$.ajax({
		...
		success : function(response) {
			if (response.status == "SAVE_SUCCESS") {
				$('#notificationNotesSuccessModalTitle').text("Success");
				$('#notificationNotesSuccessModalText').text("Review Shared Successfully !");
				$('#notificationNotesSuccessModal').modal('show');
				...
				if(response.status == "SAVE_ERROR"){
					$('#notificationNotesSuccessModalTitle').text("ALERT");
					$('#notificationNotesSuccessModalText').text("something went wrong please contact admin !");
					$('#notificationNotesSuccessModal').modal('show');
				}
			}
		},
		error : function(response) {
			alert("Unable To Get IndustryType By Id.Please Contact Admin.");
		}
	});
}
...
function saveBroadcasts(reviewId) {
	...
	$.ajax({
		...
		success : function(response) {
			if (response.status == "SAVE_SUCCESS") {
				$('#notificationNotesSuccessModalTitle').text("ALERT");
				$('#notificationNotesSuccessModalText').text("Broadcast status set successfully!");
				$('#notificationNotesSuccessModal').modal('show');
			}else{
				$('#notificationNotesSuccessModalTitle').text("ALERT");
				$('#notificationNotesSuccessModalText').text("Broadcast status set successfully!");
...
function saveNoteForTasks(reviewId, obj) {
	...
	if (noteVale == "" || dateTimeValue == "" || departmentOptionValue == null
		...
		$('#notificationNotesSuccessModalTitle').text("Alert");
		$('#notificationNotesSuccessModalText').text("Mandatory fileds(*) are required !");
		$('#notificationNotesSuccessModal').modal('show');
		return;
	}
	...
	$.ajax({
		...
		error : function(response) {
			$('#notificationNotesSuccessModalTitle').text("Error");
			$('#notificationNotesSuccessModalText').text("Something went wrong , please contact admin !");
			$('#notificationNotesSuccessModal').modal('show');
		}
	});
