...
$(document).ready(function() {
	...
	poplateOrganizations(function(selectedOrgId) {
		showDashboardSocialMentionsAdminQC(selectedOrgId);
	});
});
$("#applyFilterBtn").click(function(e){
	resetFilterBar();
	var organizationId = $('#organizationName option:selected').val();
	showDashboardSocialMentionsAdminQC(organizationId);
	saveSessionDatas();
...
function getPublicMentions(){
	...
	showDashboardSocialMentionsAdminQC(orgId);
	/*resetFilterBar();*/
}
...
function showDashboardSocialMentionsAdminQC(organizationId){
	$.ajaxSetup({ cache: false });
	loadingForDashBoard();
	...
	$.ajax({
		type:"POST",
		url:"../dashboard/showDashboardSocialMentionsAdminQC.htm",
		contentType:"application/json",
...
//dileep added
function updatedShowDashboardSocialMentionsAdminQC(organizationId){
	$('#wrapper').mask('Loading...');
	$.ajaxSetup({ cache: false });
	...
	$.ajax({
		type:"POST",
		url:"../dashboard/updatedshowDashboardSocialMentionsAdminQC.htm",
		contentType:"application/json",
		data:JSON.stringify({'fromDate':fromDate, 'toDate':toDate, 'organizationId':organizationId,'pageType':pageType}),
		success:function(response){
			 filteredMentionsList=[];
...
function getMentionsPerPage(begin, end){
	...
	for(var j=0; j<sublist.length; j++){
		...
		if(sublist[j].status == 0 ){
			...
			html +='<div class="flagOptions " id="flagToggle">';
				...
				html +='<button id="ApproveReviewFlag" class="btn btn-success btn-sm ApproveReviewFlag" type="button" onclick="approveReview('+sublist[j].id+')"> Approve</button>';
				html +='<button id="RejectReviewFlag" class="btn btn-primary btn-sm RejectReviewFlag" type="button" onclick="rejectReview('+sublist[j].id+')"> Reject</button>';
				html +='<br>'
				html +='<button id="CloseReviewFlag" class="btn btn-default btn-sm CloseReviewFlag" type="button" onclick="toggleflagOptions()"> Close</button>';
				html +='</div>';
				html +='</div>';
				html +='</div>';
				html +='</div>';
  ...
  //dileep adminQc ended
	...
	if(sublist[j].mentionType.toLowerCase()=="review"){	
		...
		$('.CloseReviewFlag').click(function() {
			$(this).parents('.flagOptions').removeClass('OnSeleceActive');
		});
		$('.ApproveReviewFlag').click(function() {
			$(this).parents('.flagOptions').removeClass('OnSeleceActive');
		});
		$('.RejectReviewFlag').click(function() {
			$(this).parents('.flagOptions').removeClass('OnSeleceActive');
		});
...
function approveReview(id){
	...
	console.log("reasionForRejections : "+reasionForRejection)
	...
	$.ajax({
		type:"POST",
		url:"../dashboard/updateSocialMentionsAdminQC.htm",
		contentType:"application/json",
		data:JSON.stringify({'id':id, 'reasionForRejection':reasionForRejection}),
		success:function(response){
			if(response.status=="FLAG_SET"){
				var organizationId = $('#organizationName option:selected').val();
				updatedShowDashboardSocialMentionsAdminQC(organizationId);
			}else{
				//getPublicMentions();
			}
		},
...
function rejectReview(id){
	reectId=id;
	var modalId = $('#RejectModal');
	modalId.modal('show');
}

function reviewForReject(){
	$.ajaxSetup({ cache: false });
	var modalId = $('#RejectModal');
	...
	//console.log("duplicateReview "+duplicateReview +" languageNotCorrect "+languageNotCorrect+" reviewdeleted  "+reviewdeleted +" others "+others);
	var reasionForRejection=[];
	reasionForRejection.push(reason);	
	console.log("id: "+id);
	...
	$.ajax({
		type:"POST",
		url:"../dashboard/updateRejectSocialMentionsAdminQC.htm",
		contentType:"application/json",
		data:JSON.stringify({'id':id, 'reasionForRejection':reasionForRejection}),
		success:function(response){
			if(response.status=="FLAG_SET"){
		...
		for (var j = 0; j < resultListMentions.length; j++) {
			...
			if(resultListMentions[j].status == 0 ){
				...
				html +='<div class="flagOptions " id="flagToggle">';
					...
					html +='<div class="form-group input-group col-xs-12">';
					html +='<button id="ApproveReviewFlag" class="btn btn-success btn-sm ApproveReviewFlag" type="button" onclick="approveReview('+resultListMentions[j].id+')"> Approve</button>';
					html +='<button id="RejectReviewFlag" class="btn btn-primary btn-sm RejectReviewFlag" type="button" onclick="rejectReview('+resultListMentions[j].id+')"> Reject</button>';
					html +='<br>'
					html +='<button id="CloseReviewFlag" class="btn btn-default btn-sm CloseReviewFlag" type="button" onclick="toggleflagOptions()"> Close</button>';
					html +='</div>';
					html +='</div>';
	...
	if (FilteredtMentions.length > 0) {
		...
		if(FilteredtMentions[j].status == 0 ){
			...
			html +='<div class="flagOptions " id="flagToggle">';
				...
				html +='<div class="form-group input-group col-xs-12">';
				html +='<button id="ApproveReviewFlag" class="btn btn-success btn-sm ApproveReviewFlag" type="button" onclick="approveReview('+FilteredtMentions[j].id+')"> Approve</button>';
				html +='<button id="RejectReviewFlag" class="btn btn-primary btn-sm RejectReviewFlag" type="button" onclick="rejectReview('+FilteredtMentions[j].id+')"> Reject</button>';
				html +='<br>'
				html +='<button id="CloseReviewFlag" class="btn btn-default btn-sm CloseReviewFlag" type="button" onclick="toggleflagOptions()"> Close</button>';
				html +='</div>';
				html +='</div>';
